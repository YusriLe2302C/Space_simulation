...
## Docker (Recommended for non-Windows / clean setup)

This repo includes:
- `Dockerfile` (root) for the Node backend
- `simulation_engine/Dockerfile` for the FastAPI engine
- `frontend/Dockerfile` for a production static build served by Nginx
- `docker-compose.yml` (root) to run the full stack + MongoDB

### 1) Create env files (once)

The compose file references real `.env` files (these are gitignored). Create them from the examples:
...
Set at minimum:
- `backend/.env`: `JWT_SECRET`, `ACM_API_KEY`, `ENGINE_SECRET`
- `simulation_engine/.env`: `ENGINE_SECRET` (must match backend)
...
### 3) Seed real objects (optional but recommended)

The database starts empty. Seed it by running the seeder inside the backend container:
...
Notes:
- The seeder downloads real TLEs from the public internet; it needs outbound network access.
- If you change `ACM_RUN_ID`, ensure your frontend `VITE_ACM_RUN_ID` matches.
